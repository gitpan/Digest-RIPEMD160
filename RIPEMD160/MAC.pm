package Digest::RIPEMD160::MAC;

use Digest::RIPEMD160 0.03;
...
sub new {
	...
	$self->{'key'} = Digest::RIPEMD160->hash($self->{'key'});
    }
...
sub mac {
    ...
    my($outer) = Digest::RIPEMD160->hash($self->{'k_opad'}.$inner);
...
sub hexmac {
    ...
    my($outer) = Digest::RIPEMD160->hexhash($self->{'k_opad'}.$inner);
...
Digest::RIPEMD160::MAC - Perl extension for RIPEMD-160 MAC function

=head1 SYNOPSIS

    use Digest::RIPEMD160::MAC;
    
    $key = "This is the secret key";

    $mac = new Digest::RIPEMD160::MAC($key);
...
The B<Digest::RIPEMD160> module allows you to use the RIPEMD160
...
=head1 EXAMPLES
    ...
    $ripemd160 = new Digest::RIPEMD160;

...
$loaded = 1;
print "ok 1  (use Digest::RIPEMD160;)\n";
...
print (($ripemd160 = new Digest::RIPEMD160) ? "" : "not ", "ok 2  (new)\n");
...
$hex = Digest::RIPEMD160->hexhash($data);
print ($hex ne '' ? "" : "not ");
...
sub test {
    ...
    my($mac) = new Digest::RIPEMD160::MAC($key);
    $mac->add(@data);
    print "not " if ($mac->hexmac() ne $digest);
    print "ok $nummer (Digest::RIPEMD160::MAC std-test-vector from RFC2286)\n";
}
